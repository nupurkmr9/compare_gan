...
class ModularGAN_Aux_Task_AET_v2(AbstractGAN):
  ...
  def _split_inputs_and_generate_samples(self, features, labels, num_sub_steps):
    # print("1: ", features, labels, num_sub_steps)
    # Encode labels.
    if self.conditional:
    ...
    if self._which_eps_distr == 'trunc_normal':
        ...
        features["eps_original"] = self.eps_distr.sample([self._num_eps, self._z_dim], name="eps_original")
    elif self._which_eps_distr == 'normal':
        features["eps_original"] = tf.random.normal([self._num_eps, self._z_dim], mean=0.0, stddev=self._eps_max, name="eps_original")
    elif self._which_eps_distr == 'uniform':
        features["eps_original"] = tf.random.uniform([self._num_eps, self._z_dim], minval=self._eps_min, maxval=self._eps_max, name="eps_original")
    elif self._which_eps_distr == 'multi_label_classification':
        features["random"] = tf.random.uniform(shape=[self._num_eps, self._z_dim], minval=0.0, maxval=1.0, name="random")
        ...
        features["random"] = tf.random.uniform(shape=[self._num_eps, self._num_groups], minval=0.0, maxval=1.0, name="random")
        features["random_mask"] = tf.less(features["random"], 0.5, name="random_mask")
        features["eps_original"] = tf.where(features["random_mask"], tf.constant(self._eps_max, shape=[self._num_eps, self._num_groups]), tf.constant(self._eps_min, shape=[self._num_eps, self._num_groups]), name="eps_original")
        features["eps_original"] = tf.reshape(features["eps_original"], [self._num_eps, -1, 1], name="reshape_eps_1")
        features["eps_original"] = tf.tile(features["eps_original"], multiples=[1, 1, self._z_dim // self._num_groups], name="eps_group_tile")
        features["eps_original"] = tf.reshape(features["eps_original"], [self._num_eps, -1], name="reshape_eps_2")

    features["eps"] = tf.tile(features["eps_original"], multiples=[self._g_bs // self._num_eps, 1], name="eps")
    
    # Split inputs for sub-steps.
    fs = [(k, tf.split(features[k], num_sub_steps)) for k in features] # Convert dict to list of tuples.
    # print("2: ", fs)
    fs = [{k: v[i] for k, v in fs} for i in range(num_sub_steps)] # Convert to list of dicts - one dict for each sub-step.
    ...
    if self._experimental_joint_gen_for_disc:
      ...
      for f in fs:
        ...
        f["disc_generated"] = self.generator(f["z"][:self._d_bs], y=sampled_y, is_training=True)
        f["disc_eps_generated"] = self.generator(tf.add(f["z"], f["eps"])[:self._d_bs], y=sampled_y, is_training=True)
  ...
  def _train_discriminator(self, features, labels, step, optimizer, params):
    ...
    self.create_d_loss(features, labels, params=params)
  ...
  def _train_generator(self, features, labels, step, optimizer, params, aux_optimizer):
    # Set the random offset tensor for operations in tpu_random.py.
    # tpu_random.set_random_offset_from_features(features) # Decide later if we need to comment out this line or not.
  ...
  def model_fn(self, features, labels, params, mode):
    ...
    unroll_graph = self._experimental_force_graph_unroll or use_tpu # unroll_graph: False for GPU
    num_sub_steps = self._get_num_sub_steps(unroll_graph=unroll_graph) # num_sub_steps: 1 for GPU
    if unroll_graph:
    ...
    if not unroll_graph and self._disc_iters != 1:
      ...
      train_gen_fn = lambda: tf.cond(
          tf.equal(disc_step % self._disc_iters, 0), train_fn, lambda: (-1.0, -1.0))
  ...
  def create_d_loss(self, features, labels, params, is_training=True):
    """Build the loss tensors for discriminator.

    This method will set self.d_loss.
    ...
    if self._deprecated_split_disc_calls:
      ...
      with tf.name_scope("disc_for_fake"):
        d_fake, d_fake_logits, _ = self.discriminator(
            fake_images, y=sampled_y, is_training=is_training)
    else:
      # Compute discriminator output for real and fake images in one batch.
      all_images = tf.concat([images, fake_images], axis=0)
      ...
      d_real = d_all[:self._real_d_bs]
      d_fake = d_all[self._real_d_bs:self._real_d_bs + 2*self._d_bs]
      # d_real_logits, d_fake_logits, d_fake_logits_eps = tf.split(d_all_logits, 3)
      d_real_logits = d_all_logits[:self._real_d_bs]
      d_fake_logits = d_all_logits[self._real_d_bs:self._real_d_bs + 2*self._d_bs]
    ...
    penalty_loss = penalty_lib.get_penalty_loss(
        x=images, x_fake=fake_images, y=y, is_training=is_training,
        discriminator=self.discriminator)
    self.d_loss += self._lambda * penalty_loss

  def create_g_loss(self, features, labels, params, is_training=True):
    """Build the loss tensors for generator and auxiliary network.

    This method will set self.g_loss and self.aux_loss.
    ...
    if self.conditional: # Not modified to have different batch sizes for d_real, d_fake and g.
      sampled_y = self._get_one_hot_labels(features["sampled_labels"])
      all_y = tf.concat([sampled_y, sampled_y], axis=0)
    else:
      sampled_y = None
      all_y = None
    ...
    if self._which_eps_distr == 'multi_label_classification':
        ...
        features["labels"] = tf.where(features["labels_mask"], tf.constant(0.0, shape=[self._g_bs, self._z_dim]), tf.constant(1.0, shape=[self._g_bs, self._z_dim]), name="labels")
        self.aux_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=pred_eps, labels=features["labels"], name="aux_cross_entropy"))
        ...
        features["labels"] = tf.cast(features["random_mask"], tf.float32, name="labels")
        self.aux_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=pred_eps, labels=features["labels"], name="aux_cross_entropy"))
        self.aux_acc = tf.reduce_mean(tf.cast(tf.equal(features["labels"], tf.round(tf.sigmoid(pred_eps))), dtype=tf.float32))
        self.g_loss += self._lambda_bce_loss * self.aux_loss
        return
